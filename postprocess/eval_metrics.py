...
def get_scores(result_file, data_file):
    ...
    for index, row in res_pd.iterrows():
        ...
        pred = results[index]
        if pred is None:
            pred = np.random.randint(0, len(row['choices']))
        else:
...
def print_scores_table(scores):
    keys = ["natural", "social", "language", 
            "has_text", "has_image", "no_context",
            "grade_1_6", "grade_7_12",
            "average"]
    latex_output = ""
    for k in keys:
        k = 'acc_' + k
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_file', type=str, default="/mnt/seagate12t/VQA/scienceqa/problems.json")
