...
prompt = llama.format_prompt("Identify the question that Kathleen and Bryant's experiment can best answer.",  'Your answer should be selected of this list:' + str({0: "Does Kathleen's snowboard slide down a hill in less time when it has a layer of wax or when it does not have a layer of wax?", 1: "Does Kathleen's snowboard slide down a hill in less time when it has a thin layer of wax or a thick layer of wax?"}))

# img = Image.fromarray(cv2.imread("../docs/logo_v1.png"))
img = Image.fromarray(cv2.imread("./ScienceQA_Data/train/3/image.png"))
img = preprocess(img).unsqueeze(0).to(device)

result, choice = model.generate(img, [prompt], ["Does Kathleen's snowboard slide down a hill in less time when it has a layer of wax or when it does not have a layer of wax?", "Does Kathleen's snowboard slide down a hill in less time when it has a thin layer of wax or a thick layer of wax?"])
