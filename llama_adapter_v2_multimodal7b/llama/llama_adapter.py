...
class LLaMA_adapter(nn.Module):
    ...
    def __init__(self, llama_ckpt_dir, llama_tokenizer,
        ...
        with open(os.path.join(llama_ckpt_dir, "params.json"), "r") as f:
            print('f:', os.path.join(llama_ckpt_dir, "params.json"))
    ...
    def forward(self, tokens, labels, imgs):
        ...
        # print('LLaMA_Adapter Output:', output) ##
        # probs = torch.softmax(output / 0.1, dim=-1)
        # print('Probs:', probs)
        # # next_token = sample_top_p(probs, 0.75)
        # next_token = torch.argmax(output, dim=-1)
        # next_token = next_token.reshape(-1)
        # print('Next_Token:', next_token)
        # # try:
        # #     t = next_token[: next_token.index(self.tokenizer.eos_id)]
        # # except ValueError:
        # #     pass
    ...
    @torch.inference_mode()
    def generate(
        self, imgs, prompts, choices=None,
        max_gen_len: int = 512,
        ...
        # prom = [x[0] for x in prompts]
        # print('Prom:', prom)

        prompts = [self.tokenizer.encode(str(x[0]), bos=True, eos=False) for x in prompts]
        print('Prompts Encoded:', prompts)
        print('Encoded Length:', len(prompts[0]))
        ...
        # print('Prompts Length:', len(prompts[0]))
        prompts = [p[:total_len] for p in prompts]
        ...
        if choices != None:
            for i, c in enumerate(choices):
                if c in decoded[0]:
                    choice = i
                    print('Choice Catched:', i)
                    break
            if choice == None:
                choice = random.randint(0, len(choices))

        return decoded[0], choice
